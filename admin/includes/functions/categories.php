  ...
  //added by Kevin L. Shelton
    function bci_get_paths($lid, $categories_array = '', $parent_id = 0, $level = 0, $path='') {
      if (!is_array($categories_array)) {
				...
				$categories_array[0]['hidden'] = false;
			}
...
function write_category_cache_file() {
  global $languages_id;
  $cache_dir = rtrim(DIR_FS_PHPCACHE, '/');
  if (!is_dir($cache_dir)) {
    if (!mkdir($cache_dir, 0755)) return false;
  }
  if (!is_writable($cache_dir)) return false;
  $cache_file = DIR_FS_PHPCACHE . FILENAME_CATEGORY_CACHE;
  ...
  foreach ($ca as $lid => $cat_array) {
    ...
    foreach ($cat_array as $id => $cat) {
      $subtext = array();
		  if ((HIDE_HIDDEN_CAT_PRODS == 'true') && $cat['hidden']) $h[] = $cat['id'];
      foreach ($cat as $key => $value) {
        if (is_bool($value)) {
          $subtext[] = "'" . $key . "' => " . ($value ? 'true' : 'false');
        } elseif (is_array($value)) {
          $subtext[] = "'" . $key . "' => array(" . implode(', ', $value) . ")";
        } else {
          $subtext[] = "'" . $key . "' => '" . str_replace(array("\\", "'"), array("\\\\", "\\'"), $value) . "'";
        }
      }
