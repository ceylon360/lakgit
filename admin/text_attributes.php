  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        tep_redirect(tep_href_link(FILENAME_TEXT_ATTRIBUTES, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'taID=' . $products_text_attributes_id));
        break;
      case 'deleteconfirm':
        $products_text_attributes_id = tep_db_prepare_input($HTTP_GET_VARS['taID']);

        tep_db_query("delete from " . TABLE_TEXT_ATTRIBUTES . " where products_text_attributes_id = '" . (int)$products_text_attributes_id . "'");

        tep_redirect(tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page']));
        break;
    }
  }
?>
...
<?php
  ...
  while ($products_text_attributes = tep_db_fetch_array($products_text_attributes_query)) {
    if ((!isset($HTTP_GET_VARS['taID']) || (isset($HTTP_GET_VARS['taID']) && ($HTTP_GET_VARS['taID'] == $products_text_attributes['products_text_attributes_id']))) && !isset($taInfo) && (substr($action, 0, 3) != 'new')) {
    ...
    if (isset($taInfo) && is_object($taInfo) && ($products_text_attributes['products_text_attributes_id'] == $taInfo->products_text_attributes_id)) {
      echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $products_text_attributes['products_text_attributes_id'] . '&action=edit') . '\'">' . "\n";
    } else {
      echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $products_text_attributes['products_text_attributes_id']) . '\'">' . "\n";
    }
?>
                <td class="dataTableContent"><?php echo $products_text_attributes['products_text_attributes_name']; ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($taInfo) && is_object($taInfo) && ($products_text_attributes['products_text_attributes_id'] == $taInfo->products_text_attributes_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $products_text_attributes['products_text_attributes_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $products_text_attributes_split->display_count($products_text_attributes_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_TEXT_ATTRIBUTES); ?></td>
                    <td class="smallText" align="right"><?php echo $products_text_attributes_split->display_links($products_text_attributes_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page']); ?></td>
                  </tr>
                </table></td>
              </tr>
<?php
...
?>
              <tr>
                <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $taInfo->products_text_attributes_id . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    ...
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form('products_text_attributes', FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $taInfo->products_text_attributes_id . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_DELETE_INTRO);
      $contents[] = array('text' => '<br><b>' . $taInfo->products_text_attributes_name . '</b>');

      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $taInfo->products_text_attributes_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (isset($taInfo) && is_object($taInfo)) {
        $heading[] = array('text' => '<b>' . $taInfo->products_text_attributes_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $taInfo->products_text_attributes_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_TEXT_ATTRIBUTES, 'page=' . $HTTP_GET_VARS['page'] . '&taID=' . $taInfo->products_text_attributes_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
      }
      break;
  }
