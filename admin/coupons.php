    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
		  <?php echo tep_draw_form('search', FILENAME_DISCOUNT_COUPONS, '', 'get'); ?>
...
?>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
           <tr>
            <td class="main" align="right" valign="top"><br><?php echo (($form_action == 'insert') ? tep_draw_button(IMAGE_INSERT, 'plus') : tep_draw_button(IMAGE_UPDATE, 'refresh')) . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . (isset($HTTP_GET_VARS['cID']) ? '&cID=' . $HTTP_GET_VARS['cID'] : ''))); ?></td>
          </tr>
        </table></td>
		<script type="text/javascript">
$('#coupons_date_start').datepicker({
  dateFormat: 'mm/dd/yy'
});
$('#coupons_date_end').datepicker({
  dateFormat: 'mm/dd/yy'
});
</script>
      </form></tr>
...
if (isset($HTTP_GET_VARS['search']) && tep_not_null($HTTP_GET_VARS['search'])) {
      $keywords = tep_db_input(tep_db_prepare_input($HTTP_GET_VARS['search']));
       $search =  "where cd.coupons_description like '%" . $keywords . "%' or cd.coupons_id like '%" . $keywords . "%'";
	} 
    $coupons_query_raw = "select * from " . TABLE_DISCOUNT_COUPONS . " cd ". $search . "order by cd.coupons_date_end, coupons_date_start";
    //end of search query... sb//
...
?>
                  <tr>
                    <td align="right"><?php echo tep_draw_button(IMAGE_NEW_COUPON, 'plus', tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . '&action=new'), 'primary'); ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_draw_button(IMAGE_DELETE, 'trash') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupons_id)));
	  break;
    default:
      if (is_object($cInfo)) {
        ...
        $contents[] = array('align' => 'center',
        										'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupons_id . '&action=edit'))
        															.tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupons_id . '&action=delete'))
        															//exclusions
        															.tep_draw_button(IMAGE_PRODUCT_EXCLUSIONS, 'triangle-1-e', tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=products'))
        															.tep_draw_button(IMAGE_MANUFACTURER_EXCLUSIONS, 'triangle-1-e', tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=manufacturers'))
        															.tep_draw_button(IMAGE_CATEGORY_EXCLUSIONS, 'triangle-1-e', tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=categories'))
        															.tep_draw_button(IMAGE_CUSTOMER_EXCLUSIONS, 'triangle-1-e', tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=customers'))
																	.tep_draw_button(IMAGE_SHIPPING_ZONE_EXCLUSIONS, 'triangle-1-e', tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=zones'))
        															//end exclusions
        									 );
