...
if(isset($_GET[AM_ACTION_GET_VARIABLE]) && $_GET[AM_ACTION_GET_VARIABLE] == 'prompt') {
	...
	switch($_GET['section']) {
		case 'amAddOption':
			...
			$amPopup->addToContents(updateCancelButtons(tep_db_prepare_input($_GET['section'])));
			echo $amPopup->output();
			break;
		...
		case 'amRemoveOptionFromProduct': 
			...
			$amPopup->addToContents(yesNoButtons(tep_db_prepare_input($_GET['section'])));
			$amPopup->addToContents(tep_draw_hidden_field('option_id',$arrExtraValues['option_id'],'id="option_id"'));
			echo $amPopup->output();
			break;
...
*/			// EOF only for TinyMCE filemanager
			...
			//$amPopup->addToContents(tep_db_prepare_input($_GET['section']));
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents(updateCancelButtons(tep_db_prepare_input($_GET['section'])));
			$amPopup->addToContents(tep_draw_hidden_field('option_id',$arrExtraValues['option_id'],'id="option_id"'));
