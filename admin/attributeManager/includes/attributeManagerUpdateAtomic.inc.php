...
if(is_array(${AM_SESSION_VAR_NAME}) && is_numeric($products_id)){
	foreach(${AM_SESSION_VAR_NAME} as $newAttribute) {
		...
		$newAttributeData = array(
			...
			// Product Attributre Code
            if (AM_USE_ATTRIBUTE_CODE) {
              $newAttributeData['code_suffix'] = amDB::input($newAttribute['code_suffix']);
			  if (AM_USE_ATTRIBUTE_CODE_SORT) {
				$newAttributeData['suffix_sort_order'] = amDB::input($newAttribute['suffix_sort_order']);
			   }
            }
