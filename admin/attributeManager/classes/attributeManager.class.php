		...
		if(0 === count($this->arrAllTemplatesAndAttributes)) {
			...
			foreach($ref as $res) {
                ...
                if (AM_USE_ATTRIBUTE_CODE) {
                  $this->arrAllTemplatesAndAttributes[$templateID]['code_suffix'][$res['option_values_id']] = $res['code_suffix'];
				  if (AM_USE_ATTRIBUTE_CODE_SORT) {
					$this->arrAllTemplatesAndAttributes[$templateID]['suffix_sort_order'][$res['option_values_id']] = $res['suffix_sort_order'];
				  }
                }
           }
		}
			...
			foreach($allProdOptionsAndValues as $optionId => $values) {
				if(is_array($values['values'])) {
					foreach ($values['values'] as $optionValuesId => $allOptionValues){
						...
						// Product Attribute Code
                        if (AM_USE_ATTRIBUTE_CODE) {
                          $data['code_suffix'] = $values['values'][$optionValuesId]['code_suffix'];
						  if (AM_USE_ATTRIBUTE_CODE_SORT) {
							$data['suffix_sort_order'] = $values['values'][$optionValuesId]['suffix_sort_order'];
						  }
                        }
