...
// Product Attribute Code
  if (AM_USE_ATTRIBUTE_CODE) {
    if(isset($get['code_suffix']) === true) {
      $this->getAndPrepare('code_suffix', $get, $getArray['code_suffix']);
    }
	if (AM_USE_ATTRIBUTE_CODE_SORT) {
		if(isset($get['suffix_sort_order']) === true) {
		  $this->getAndPrepare('suffix_sort_order', $get, $getArray['suffix_sort_order']);
		}
	}
  }
...
// Product Attribute Code
  if (AM_USE_ATTRIBUTE_CODE) {
    if((empty($getArray['code_suffix']))||($getArray['code_suffix']==' ')){
      $getArray['code_suffix']='';
    }
	if (AM_USE_ATTRIBUTE_CODE_SORT) {
		if((empty($getArray['suffix_sort_order']))||($getArray['suffix_sort_order']==' ')){
			$getArray['suffix_sort_order']='';
		}
	}
  }
...
// Product Attribute Code		  
		...
		foreach($this->arrSessionVar as $id => $res) {
			if(($res['option_id'] == $getArray['option_id']) && ($res['option_value_id'] == $getArray['option_value_id'])) {
				...
				// Product Attribute Code
                if (AM_USE_ATTRIBUTE_CODE) {
                  $this->arrSessionVar[$id]['code_suffix'] = $getArray['code_suffix'];
				  if (AM_USE_ATTRIBUTE_CODE_SORT) {
					$this->arrSessionVar[$id]['suffix_sort_order'] = $getArray['suffix_sort_order'];
				  }
                }
	...
	function getAllProductOptionsAndValues($reset = false) {
		...
		for($z1=0;$z1<count($this->arrSessionVar);$z1++){
			...
			foreach($this->arrSessionVar as $id => $res) {
				...
				//Product Attribute Code
                if (AM_USE_ATTRIBUTE_CODE) {
                  $this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['code_suffix'] = $res['code_suffix'];
				  if (AM_USE_ATTRIBUTE_CODE_SORT) {
					$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['suffix_sort_order'] = $res['suffix_sort_order'];
				  }
                }
