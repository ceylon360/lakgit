...
function amUpdate(optionId, optionValueId, optionSender) {
    ...
    code_suffix=getDropDownValue('code_suffix_'+optionValueId);
    suffix_sort_order=getDropDownValue('suffix_sort_order_'+optionValueId);
	...
	// Product Attribute Code
	if (code_suffix != null) getElement('code_suffix_'+optionValueId).blur();
	if (suffix_sort_order != null) getElement('suffix_sort_order_'+optionValueId).blur();
...
function amAddAttributeToProduct() {
  ...
  // Product Attribute Code
  var code_suffix = getDropDownValue('newCodeSuffix');
  var suffix_sort_order = '';//getDropDownValue('newSuffixSortOrder');
//	var sortOrder = getDropDownValue('newSort');
	var sortOrder = -1;
	
	if(0 == option || 0 == optionValue)
		return false;
	amSendRequest('amAction=addAttributeToProduct&option_id='+option+'&option_value_id='+optionValue+'&prefix='+pricePrefix+'&price='+price+'&sortOrder='+sortOrder+'&weight_prefix='+weightPrefix+'&weight='+weight+'&code_suffix='+code_suffix+'&suffix_sort_order='+suffix_sort_order);
	return false;
}
