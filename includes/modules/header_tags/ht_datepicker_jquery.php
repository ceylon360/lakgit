    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_DATEPICKER_JQUERY_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
		 ...
		 // $oscTemplate->addBlock('<link rel="stylesheet" href="ext/datepicker/css/bootstrap-datepicker.css" />' . "\n", 'header_tags');
          // create_account
          // account edit
          //$oscTemplate->addBlock('<script>$(\'#dob\').datepicker({dateFormat: \'' . JQUERY_DATEPICKER_FORMAT . '\',viewMode: 2});</script>', $this->group);
		  //test
		//$oscTemplate->addBlock('<script>$(\'#datepicker\').datepicker({daysOfWeekDisabled:"06"});</script>', $this->group);
		//  $oscTemplate->addBlock('<script>$(\'#datetimepicker1\').datepicker();</script>', $this->group);
		//  $oscTemplate->addBlock('<script>$(\'#datetimepicker1\').on("changeDate", function(event) { $(\'#my_hidden_input\').val($(\'#datetimepicker1\').datepicker(\'getFormattedDate\'))});</script>', $this->group);
		//  $oscTemplate->addBlock('<script>$(\'#delivery_date\').datetimepicker({dateFormat: \'' . JQUERY_DATEPICKER_FORMAT2 . '\',viewMode: 0});</script>', $this->group);
          // advanced search
