  ...
  class bm_reviews {
    ...
    function execute() {
     /* global $languages_id, $HTTP_GET_VARS, $currencies, $oscTemplate;
	  ...
	  //cat state
	  global $languages_id, $HTTP_GET_VARS, $currencies, $oscTemplate, $hiddencats;
 
      if (!empty($hiddencats)) {
        $random_select = "select r.reviews_id, r.reviews_rating, p.products_id, p.products_image, pd.products_name from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_status = '1' and p.products_id = r.products_id and r.reviews_id = rd.reviews_id and rd.languages_id = '" . (int)$languages_id . "' and p.products_id = pd.products_id and p.products_id = p2c.products_id and pd.language_id = '" . (int)$languages_id . "' and (not (p2c.categories_id in (" . implode(',', $hiddencats) . ")))";
      } else {
        $random_select = "select r.reviews_id, r.reviews_rating, p.products_id, p.products_image, pd.products_name from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = r.products_id and r.reviews_id = rd.reviews_id and rd.languages_id = '" . (int)$languages_id . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and r.reviews_status = 1";
      }
      if (isset($HTTP_GET_VARS['products_id'])) {
      ...
      $random_select .= " group by p.products_id order by r.reviews_id desc limit " . MAX_RANDOM_SELECT_REVIEWS;
	  //cat state
