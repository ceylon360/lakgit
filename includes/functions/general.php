  ...
  function tep_count_products_in_category($category_id, $include_inactive = false) {
    ...
    if ($include_inactive == true) {
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$category_id . "'");
		...
		//cat state
		if (is_array($category_info_array) && !empty($category_info_array)) return $category_info_array[$category_id]['prod_count'];
		//cat state
...
// TABLES: categories
  function tep_has_category_subcategories($category_id) {
	  //cat state
	  global $category_info_array;
    if (is_array($category_info_array) && !empty($category_info_array)) return $category_info_array[$category_id]['has_subcat'];
	  //cat state
